...
public class OilScannerItem extends Item {
    public OilScannerItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (!level.isClientSide) {
            ...
            for (int dx = -scanRadius; dx <= scanRadius; dx++) {
                for (int dz = -scanRadius; dz <= scanRadius; dz++) {
                    for (int dy = 0; dy <= maxDepth; dy++) {
                        ...
                        if (state.is(DefensiveBlocks.DIRTY_RAW_CRUDE_OIL.get())) {
            ...
            if (nearestOil == null || Math.sqrt(closestDistanceSq) > 250) {
                spawnParticles(level, playerPos, ParticleTypes.SMOKE, 12, 0.2);
                player.sendSystemMessage(Component.literal("§cNo Oil Nearby"));
            } else {
                BlockPos oilXZ = new BlockPos(nearestOil.getX(), playerPos.getY(), nearestOil.getZ());
                int dx = oilXZ.getX() - playerPos.getX();
                int dz = oilXZ.getZ() - playerPos.getZ();
                ...
                if (horizontalDist > 100) {
                    ...
                    player.sendSystemMessage(Component.literal("§aOil Found"));
                }
            }
        }
